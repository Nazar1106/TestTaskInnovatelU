        ...
        return documentStorage.values().stream()
                .filter(document -> request.getTitlePrefixes() == null ||
                        request.getTitlePrefixes()
                                .stream().anyMatch(title -> document.getTitle().startsWith(title)))
                ...
                .filter(document -> request.getAuthorIds() == null ||
                        request.getAuthorIds().contains(document.getAuthor().id))
                .filter(document -> request.getCreatedFrom() == null ||
                        request.getCreatedFrom().isBefore(document.created))
                .filter(document -> request.getCreatedTo().isAfter(document.created)).toList();
    }
